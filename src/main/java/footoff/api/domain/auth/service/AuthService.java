...
/**
 * 인증 관련 서비스 인터페이스
 * 소셜 로그인(카카오, 애플 등) 인증 처리와 계정 생성을 위한 서비스를 정의합니다.
 */
public interface AuthService {
     ...
     * 애플 로그인 처리
     * 
     * 애플 인증 코드를 이용하여 로그인을 처리하고 사용자 정보를 반환합니다.
     * 로그인 과정에서 액세스 토큰을 발급받고, 사용자 프로필 정보를 가져와 계정을 생성하거나 조회합니다.
     * 로그인 성공 시 JWT 토큰을 생성하여 응답 헤더에 추가합니다.
     *
     * @param code 애플 인증 코드 (애플 인증 서버에서 리다이렉트로 받은 코드)
     * @param httpServletResponse HTTP 응답 객체 (JWT 토큰을 헤더에 추가하기 위해 사용)
     * @return 애플 로그인 응답 DTO (사용자 ID와 애플 토큰 정보 포함)
     */
    AppleLoginResponseDto appleLogin(String code, HttpServletResponse httpServletResponse);
     ...
     * 애플 계정 생성
     * 
     * 애플 ID를 이용하여 새로운 사용자 계정과 소셜 계정 정보를 생성합니다.
     * 이 메서드는 애플 로그인 시 해당 ID로 등록된 계정이 없을 때 호출됩니다.
     *
     * @param appleId 애플 사용자 고유 ID
     * @param email 사용자 이메일 (없는 경우 빈 문자열)
     * @return 생성된 소셜 계정 엔티티
     */
    UserSocialAccount createAppleAccount(String appleId, String email);
} 